...
output: 
  rmarkdown::html_vignette: 
    keep_md: TRUE
vignette: >
  %\VignetteIndexEntry{Introcution to the rGEDI package}
...
lasfile_cerrado <- file.path(outdir, "Cerrado.las")
...
wf_cerrado<-gediWFSimulator(
  ...
  output = file.path(getwd(),"/gediWF_cerrado_simulation.h5"),
...
wf_cerrado_metrics<-gediWFMetrics(
  input   = wf_cerrado,
  outRoot = file.path(getwd(), "cerrado"))
...
```{r cleanup, echo=TRUE, results="hide", error=TRUE, eval=FALSE}
close(wf_amazon)
close(wf_cerrado)
close(gedilevel1b)
close(gedilevel2a)
