...
#'
#! \bold{Input output}
#' @param input [`rGEDI::gedi.level1b-class`] (may be a list of objects). Simulated waveform input object(s).
...
# Always GEDI
#' @param readBinLVIS input is an LVIS binary file. Default FALSE.
#' @param readHDFlvis read LVIS HDF5 input. Default FALSE.
#' @param readHDFgedi read GEDI simulator HDF5 input. Default TRUE.
#' @param level2 name. level2 filename for LVIS ZG. Default NULL.
...
gediWFMetrics = function(
  ...
  stopifnotMessage(
    "Input file is not gedi.level1b or list"=class(input) == "gedi.level1b" ||
                                                      all(sapply(input, class) == "gedi.level1b"),
    "outRoot is not a valid path!"=checkParentDir(outRoot, optional=FALSE),
