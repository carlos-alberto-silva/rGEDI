...
#'
#' @slot h5 Object of class H5File from `hdf5r` package containing the
#' GEDI Level 4A (L4A): Predictions of the aboveground biomass density (AGBD; in Mg/ha) and
#' estimates of the prediction standard error within each sampled geolocated laser footprint.
#'
#' @seealso [`H5File`][hdf5r::H5File-class] in the `hdf5r` package and
...
#'
#' @param method methods used for simulating the GEDI full-waveform
#' ("RXWAVEINT", "RXWAVECOUNT" or "RXWAVEFRAC"). Default is "RXWAVECOUNT".
...
setMethod(
  "plot",
  signature("gedi.level1bSim", y = "missing"),
  function(x, relative = FALSE, polygon = FALSE, method = "RXWAVEINT", ...) {
    if (!class(x) == "gedi.level1bSim") {
      ...
      if (method == "RXWAVECOUNT") {
        x0 <- wfh5[["RXWAVECOUNT"]][, ]
      }
      if (method == "RXWAVEFRAC") {
        x0 <- wfh5[["RXWAVEFRAC"]][, ]
      }
